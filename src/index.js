...
function printCurrencyNames(code) {
  ...
  outputDiv.querySelector(
    ".base-code"
  ).innerText = `Conversion for ${code[0].toUpperCase()} to ${code[1].toUpperCase()}`;
}

function printResults(response, amount) {
  const outputDiv = document.querySelector("#output");
  outputDiv.querySelector(".amount-entered").innerText = `${
    response.base_code
  }: $${displayBigNums(financial(amount))}`;
  outputDiv.querySelector(
    ".exg-rate"
  ).innerText = `Conversion Rate: ${response.conversion_rate}`;
  outputDiv.querySelector(".exg-result").innerText = `${
    response.target_code
  }: $${displayBigNums(financial(response.conversion_result))}`;
}
...
function handleSubmitEvent(e) {
  e.preventDefault();
  resetError();
  const baseCode = checkTextInputs(
    document.querySelector("input[list='base-code']").value
  );
  const targetCode = checkTextInputs(
    document.querySelector("input[list='target-code']").value
  );
...
window.addEventListener("load", function () {
  this.document
    .querySelector("form")
    .addEventListener("submit", handleSubmitEvent);
  getCountryCodes();
});
